...
public function buscar_cliente($txt){
    ...
    public function show_pdf($id,$opc_id,$correo,$etiqueta){
			...
			$rst_opc=$this->opcion_model->lista_una_opcion($opc_id);
			$rst_cja=$this->caja_model->lista_una_caja($rst_opc->opc_caja);
			$rst_cja_sri=$this->caja_model->lista_una_caja($rst->cja_id);
			$cns_pg=$this->factura_model->lista_pagos_factura($id);
			$cns_pag=array();
			...
			if($rst_cja_sri->cja_id==1){
                   ...
                   $tipo ='Nota de venta'; 
                   $leyenda_pie ='*Este es un documento sin validez tributaria';	
                   $hidden ='hidden';
                   $doc='pdf_nota_venta';
    		...
    		$this->html3pdf->folder($_SERVER['DOCUMENT_ROOT']."/almacencotopaxi/pdfs/");
            $this->html3pdf->filename($rst->fac_clave_acceso.'.pdf');
            $this->html3pdf->create('save');
            
			if(empty($correo)){
				$this->html3pdf->output(array("Attachment" => 0));	
			}else if($correo==1){
    ...
    public function show_talon($id,$opc_id,$correo,$etiqueta){
			...
			//$this->html3pdf->paper(array(204.75,533.66), 'portrait');
    		// $this->html3pdf->html(utf8_decode($this->load->view('pdf/pdf_talon', $data, true)));
    		// $this->html3pdf->output(array("Attachment" => 0));	
    ...
    public function consulta_api($dato){
		...
		if(($rst)!=false){
			$data=array(
						'cli_raz_social'   =>$rst->names,
						'cli_ced_ruc'   =>$ci,
						'cli_nom_comercial'=>$rst->names
						);
			echo json_encode($data);
		}else{
			echo "";
    ...
    public function consulta_sri($id,$opc_id){
    	...
    	if($ambiente!=0){
	        ...
	        if (empty($result['RespuestaAutorizacionComprobante']['autorizaciones'])) {
	           $this->generar_xml($factura->fac_id); 
	        } else {
    ...
    function envio_mail($datos){
        ...
        $this->email->subject("DOCUMENTOS ELECTRONICOS $datos->tipo No: $datos->numero Cliente : $datos->cliente ");
